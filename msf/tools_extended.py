...
class ConsoleExtendedTools(MSFConsoleStableWrapper):
                ...
                workflow_steps.append(
                    {
                        ...
                        "session": (
                            exploit_result.data.get("session") if exploit_result.data else None
                        ),
                    ...
                    workflow_steps.append(
                        {
                            ...
                            "sessions": (
                                sessions_result.data.get("count", 0) if sessions_result.data else 0
                            ),
                ...
                if result.status == OperationStatus.SUCCESS:
                    return ExtendedToolResult(
                        ...
                        data={
                            ...
                            "path": f"/tmp/creds.{export_format}",
                        },
                        execution_time=result.execution_time,
                    )
            ...
            if action == "list":
                ...
                if loot_type and loot_type in loot_modules:
                    ...
                    post_result = await self.msf_post_exploitation(
                        session_id=session_id, module=module, timeout=timeout
                    )
        ...
        try:
            if action == "list":
                ...
                result = await self.execute_command(
                    f"db_import {vuln_data['file']}", timeout or 60.0
                )
        ...
        try:
            ...
            if report_type == "hosts":
                ...
                report_data["data"] = {
                    ...
                    "timeline": self._build_exploitation_timeline(
                        sessions_result.data, loot_result.data
                    ),
                }
                ...
                report_data["data"] = {
                    "summary": {
                        "total_hosts": (
                            hosts_result.data.get("count", 0) if hosts_result.data else 0
                        ),
                        "total_services": (
                            services_result.data.get("count", 0) if services_result.data else 0
                        ),
                        "total_vulnerabilities": (
                            vulns_result.data.get("count", 0) if vulns_result.data else 0
                        ),
                        "active_sessions": (
                            sessions_result.data.get("count", 0) if sessions_result.data else 0
                        ),
        ...
        try:
            ...
            if execute:
                ...
                for step_info in workflow_script:
                    ...
                    if result.status == OperationStatus.FAILURE and not step.get(
    ...
    def _parse_options(self, output: str) -> Dict[str, Any]:
        ...
        options = {"module_options": [], "payload_options": [], "advanced_options": []}
    ...
    def _parse_check_result(self, output: str) -> Dict[str, Any]:
        ...
        result = {"vulnerable": False, "status": "unknown", "details": ""}
    ...
    def _parse_sessions(self, output: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if (
                not line
                or line.startswith("Active sessions")
                or line.startswith("=")
    ...
    def _parse_post_module_output(self, module: str, output: str) -> Dict[str, Any]:
        ...
        results = {"success": True, "data": [], "raw_output": output}
    ...
    def _parse_jobs(self, output: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if len(parts) >= 2 and parts[0].isdigit():
                job = {"id": int(parts[0]), "name": " ".join(parts[1:])}
                jobs.append(job)
        ...
        for line in lines:
            ...
            if scan_type == "port":
                if "open" in line.lower():
                    parts = line.split()
                    if len(parts) >= 3:
                        results.append({"host": parts[0], "port": parts[1], "state": "open"})
    ...
    def _parse_routes(self, output: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if (
                not line
                or line.startswith("IPv4 Active Routing")
                or line.startswith("=")
                or line.startswith("Subnet")
            ):
                continue
            ...
            parts = line.split()
            if len(parts) >= 3:
                route = {"subnet": parts[0], "netmask": parts[1], "gateway": parts[2]}
                routes.append(route)
    ...
    def _parse_vulnerabilities(self, output: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if (
                not line
                or line.startswith("Vulnerabilities")
                or line.startswith("=")
    ...
    def _build_exploitation_timeline(
        self, sessions_data: Dict, loot_data: Dict
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_risk_score(self, vulns_data: Dict) -> Dict[str, Any]:
        ...
        return {"score": score, "level": level, "breakdown": severity_count}
    ...
    def _generate_recommendations(self, vulns_data: Dict) -> List[str]:
        ...
        if severity_count["critical"] > 0:
            recommendations.append(
                f"URGENT: Patch {severity_count['critical']} critical vulnerabilities immediately"
            )

        if severity_count["high"] > 0:
            recommendations.append(
                f"HIGH PRIORITY: Address {severity_count['high']} high-severity vulnerabilities"
            )
        ...
        for service, count in service_count.items():
            if count > 0:
                recommendations.append(
                    f"Review and harden {service.upper()} service configuration ({count} vulnerabilities)"
                )
    ...
    def _parse_plugins(self, output: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            # Parse plugin line
            if line:
                plugins.append({"name": line, "loaded": True})
    ...
    async def test_extended_tools():
        ...
        if init_result.status == OperationStatus.SUCCESS:
            ...
            module_result = await tools.msf_module_manager(
                action="info", module_path="exploit/windows/smb/ms17_010_eternalblue"
            )
